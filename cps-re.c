...
char *cpsre_matchend(char *regex, char *input) {
...
char *cpsre_matchbegin(char *regex, char *input) {
  for (char *end; *input; input++)
    if ((end = cpsre_matchend(regex, input)) != NULL)
      return end == CPSRE_SYNTAX ? CPSRE_SYNTAX : input;
  return cpsre_matchend(regex, input);
}
